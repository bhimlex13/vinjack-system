...
// --- MODIFIED: This function now ARCHIVES, not deletes ---
const deleteProduct = async (req, res) => {
    try {
        const product = await Product.findById(req.params.id);
        if (product) {
            // --- Don't delete. Set status to inactive ---
            product.status = 'inactive';
            await product.save();
            
            // --- Update log action type ---
            logAction(
              req.user, 
              'ARCHIVE_PRODUCT', // <-- Changed log type
...
const getProductsBySupplier = async (req, res) => {
  try {
    ...
    const products = await Product.find({ 
        ...
        .select('itemCode name supplierCosts.$'); 
     ...
     const productsWithCost = products.map(p => {
         ...
         return {
             ...
             itemCode: p.itemCode,
             name: p.name,
             cost: relevantCostEntry ? relevantCostEntry.cost : 0 
         };
     });
